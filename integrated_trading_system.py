...
class TradingEngine:
    ...
    def run(self):
        ...
        try:
            while self._running:
                ...
                total_value = self.bankroll + exposure + unrealized
                total_pnl = total_value - self.starting_bankroll
                ...
                logger.info(f"💰 Portfolio: ${total_value:,.2f} | P&L: ${total_pnl:+,.2f} ({pnl_pct:+.1f}%)")
                logger.info(f"📊 Positions: {len(self.positions)} open | Bets: {self.daily_bets}")
                logger.info("=" * 60)
                ...
                if not self._check_risk_limits():
                    logger.info("⚠️ Risk limits hit - waiting...")
                    time.sleep(self.config.scan_interval_secs)
                    continue
                
                try:
                    opportunities = self._scan_markets()
                    logger.info(f"Found {len(opportunities)} opportunities with edge >= {self.config.min_edge*100:.0f}%")
                ...
                logger.info(f"💤 Sleeping {self.config.scan_interval_secs}s...")
                time.sleep(self.config.scan_interval_secs)
    ...
    def _check_risk_limits(self) -> bool:
        ...
        position_value = sum(p.contracts * p.current_price / 100 for p in self.positions)
        total_value = self.bankroll + position_value
        
        # Drawdown is from starting bankroll to current total value
        drawdown = (self.starting_bankroll - total_value) / self.starting_bankroll if self.starting_bankroll > 0 else 0
        
        if drawdown >= self.config.max_drawdown_pct:
            logger.warning(f"  Drawdown limit: {drawdown:.1%} >= {self.config.max_drawdown_pct:.1%}")
            return False
        if self.daily_pnl <= -self.config.max_daily_loss:
            logger.warning(f"  Daily loss limit: ${self.daily_pnl:.2f} <= -${self.config.max_daily_loss:.2f}")
            return False
        if len(self.positions) >= self.config.max_open_positions:
            logger.warning(f"  Max positions: {len(self.positions)} >= {self.config.max_open_positions}")
            return False
        if self.daily_bets >= self.config.max_daily_bets:
            logger.warning(f"  Max daily bets: {self.daily_bets} >= {self.config.max_daily_bets}")
            return False
        return True
